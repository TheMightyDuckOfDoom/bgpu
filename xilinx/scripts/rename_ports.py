...
uniqueifier = "_vivadosynth"

def rename(name):
    return name.replace("[", "_BRA_").replace("]", "_KET_").replace(".", "_DOT_")
...
for module in data['modules']:
    ...
    for cell_name in data['modules'][module]['cells']:
        ...
        # If the module is in the netlist, append the uniqueifier
        if cell['type'] in data['modules']:
            cell['type'] = cell['type'] + uniqueifier
    ...
    if module != top_module:
        new_module_name = module + uniqueifier
